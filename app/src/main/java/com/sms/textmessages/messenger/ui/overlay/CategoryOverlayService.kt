...
// design in CategoryOverlayCard. Reuses the same WindowManager.LayoutParams shape the
// removed OverlayAdService used (TYPE_APPLICATION_OVERLAY, FLAG_NOT_FOCUSABLE),
// but hosts a Compose tree via ComposeView instead of an inflated XML layout,
// which needs manual Lifecycle/SavedStateRegistry/ViewModelStore owners since
// a bare Service provides none of the three ComposeView requires.
class CategoryOverlayService : LifecycleService(), SavedStateRegistryOwner, ViewModelStoreOwner {
    ...
    private fun showOverlay(category: NotificationCategory, sender: String, body: String, notificationId: Int) {
        ...
        try {
            ...
            Log.e("CATEGORY_OVERLAY", "Failed to add overlay view", e)
            stopSelf()
            return
        }
    ...
    private fun removeOverlay() {
        ...
        try {
            ...
            Log.e("CATEGORY_OVERLAY", "Failed to remove overlay view", e)
        }
        overlayView = null
    }
